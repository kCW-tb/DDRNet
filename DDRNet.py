...
class segmenthead(nn.Module):
    def __init__(self, inplanes, interplanes, outplanes, scale_factor=8):
        ...
        self.scale_factor = scale_factor # 원본 이미지 크기 복원 배율
...
class DDRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ = self.layer3_(self.relu(layers[1])) # 고해상도
        x = self.layer3(self.relu(x))           # 저해상도
        layers.append(x)
        ...
        x_ = self.layer4_(self.relu(x_)) # 고해상도
        x = self.layer4(self.relu(x))   # 저해상도
        layers.append(x)
        ...
        x_ = self.layer5_(self.relu(x_)) # 고해상도
        x = self.layer5(self.relu(x))   # 저해상도
        x = self.spp(x) # 저해상도에 SPP 적용
        x = F.interpolate(
                        x,
                        size=[height_output, width_output],
                        mode='bilinear', align_corners=True)
